...
export interface LoggerConfig {
  ...
  format?: Partial<
    Record<LogEventFormatSections, string | ((value: any) => string)>
  >;
  delimiter: string;
}
...
export function createLoggerFor(
  namespace: string,
  config?: Partial<LoggerConfig>,
): Logger {
  let currentConfig = config;
  const getConfig = () =>
    deepAssign<LoggerConfig>({}, currentGlobalConfig, currentConfig);
  const result: Logger = handleLog.bind(
    this,
    getConfig,
    namespace,
    undefined,
  ) as any as Logger;
  Object.values(LogLevel).forEach(
    (level) =>
      (result[level] = handleLog.bind(this, getConfig, namespace, level)),
  );
...
function formatLog(config: LoggerConfig, event: LogEvent): any[] {
  ...
  return [
    result.join(nullishCoalesce(config.delimiter, " ")),
    ...styles,
    ...event.extras,
  ];
}
